...
function showTnc() {
  ...
  document.getElementById('tncClose').onclick = () => {
...
// ── Login — goes directly to doLogin, no T&C gate ────────────────────────────
...
function buildDashboard(data) {
  ...
  setTimeout(() => {
    ring.style.strokeDashoffset = circ - (circ * Math.min(p, 100) / 100);
  }, 150);
  ...
  document.getElementById('icNeed90').textContent     = p >= 60 ? '✓' : need60;
  document.getElementById('icNeed90').style.color     = p >= 60 ? 'var(--green)' : 'var(--blue)';
  document.getElementById('icNeed90Desc').textContent = p >= 60 ? 'Already above 60%' : `${need60} classes needed`;
...
document.getElementById('filterBtn').addEventListener('click', filterDates);
